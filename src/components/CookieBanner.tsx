...
export function CookieBanner() {
  ...
  return <CookieDialog decide={decide} />;
}

function CookieDialog({ decide }: { decide: (c: "all" | "essential") => void }) {
  const { lang } = useLang();
  return (
    <div
      role="dialog"
      aria-label={lang === "en" ? "Cookie consent" : "Consentement aux cookies"}
    ...
    >
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <p className="text-sm text-on-surface-variant">{t(lang, "cookieText")}</p>
        <div className="flex shrink-0 gap-3">
          <button onClick={() => decide("essential")} className="btn-outline whitespace-nowrap">
            {t(lang, "essentialOnly")}
          </button>
